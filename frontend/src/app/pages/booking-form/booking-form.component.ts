...
export class BookingFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const quantity = Number(this.route.snapshot.queryParams['quantity']) || 1;
    ...
    this.roomService.getRoomById(id).subscribe({
      next: (data) => {
        ...
        this.bookingForm.get('numberOfGuests')?.setValidators([
          Validators.required,
          Validators.min(1),
          Validators.max(data.maxGuests * this.bookingForm.get('quantity')?.value)
        ]);
  ...
  calculateTotals(): void {
    ...
    if (room && checkInDate && checkOutDate) {
      ...
      if (end > start) {
        const quantity = this.bookingForm.get('quantity')?.value || 1;
        const diffTime = Math.abs(end.getTime() - start.getTime());
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        this.numberOfNights.set(diffDays);
        this.totalPrice.set(diffDays * room.pricePerNight * quantity);
      } else {
